...
def decision_modules(root: Path = ROOT) -> list[str]:
    ...
    The closure is `identity.decision_modules`, which captures enforce. Research records
    `uv.lock` beside it, because a study is reproduced from a checkout and its lockfile;
    a capture instead records the dependency versions it actually ran with.
    """
    return sorted([*identity.decision_modules(root / "src" / "pool"), "uv.lock"])
